...
async function addEventsToDigest(events) {
  ...
  await prisma.pendingDigest.createMany({
...
async function sendDigest() {
  const pending = await prisma.pendingDigest.findMany({ orderBy: { createdAt: 'asc' } });
  if (!pending.length) {
  ...
  await prisma.pendingDigest.deleteMany({});
  const events = pending.map(p => p.eventData);
